...
- [ Локальный запуск (SQLite)](#локальный-запуск-sqlite)
...
4. **Конфигурация SequentialExecutor** (последовательное выполнение)
5. **Инициализация Airflow** и создание пользователя `admin`
6. **Запуск Scheduler и Webserver**

### Параметры SQLite-режима
```bash
...
ml-pipeline-project/
 ...
 Dockerfile.airflow # Кастомный Airflow образ
 optimize_docker_images.sh # Скрипт оптимизации
 ...
 Данные и результаты
 data/
 wdbc.data.csv # Исходный датасет
